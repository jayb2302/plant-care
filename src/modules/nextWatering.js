...
export function calculateNextWateringDate(lastWateredDate, waterFrequency) {
  ...
  if (match) {
    // If waterFrequency matches the 'X days' format, calculate the next watering date and days until watering.
    ...
    return {
      ...
      daysUntilWatering: Math.ceil(timeDiff / (24 * 60 * 60 * 1000))
    }
    ...
    // Calculate the average number of days between min and max.
