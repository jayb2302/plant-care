...
export const getPlantList = async (params = {}) => {
  ...
  if (params.q) {
    uri.searchParams.append('q', params.q)
  }
  if (params.page) {
  ...
  try {
    ...
    // Return full response with pagination info if page param is set, else return just the list
    return params.page ? data : data.data
  } catch (e) {
    throw new Error('Failed searching plants')
  }
}
...
export const getPlantById = async (id) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Failed fetching plant')
