<template>
  <div class="md:flex  p-3 md:h-[75vh] ">
    ...
    <div v-if="selectedSite"
      ...
      :initial="{ opacity: 0 }"
      :enter="{ opacity: 1 }"
      :leave="{ opacity: 0 }"  class="plantsList-wrapper md:z-0 relative md:w-10/12 md:bottom-10 pb-3 -top-10 md:top-5 flex flex-col  p-1">
      ...
      <div  class="plantsList-content  flex flex-col h-[60vh] md:h-[70vh] overflow-y-scroll mx-5" v-if="selectedSite">
        <div class="myplant-card  flex my-1 mx-2 flex-wrap  py-5  gap-2 justify-center">
          <div class="myplant  divide-y-2 divide-orange-900 relative h-auto p-2 w-70  md:w-60" v-for="(plant) in filteredPlants" :key="plant.id">
            <div v-motion
              :initial="{
                opacity: 0,
                y: 100,
            ...
            >
              <div class="relative flex flex-col  md:justify-center " v-if="plant.siteId === selectedSite.id">
                <div class="plant-details ">
                  ...
                  <h2 class="text-1xl md:text-2xl pl-1 capitalize truncate z-10 top-5"><strong>{{ plant.plant_nickname }}</strong></h2>
