<template>
  <div class="md:flex  p-3 md:h-[75vh] ">
    <!-- Sidebar for sites -->

    <div class="sitesSidebar lg:w-2/12 p-2 border-r absolute right-0 top-44 hidden md:block">
    ...
    <div v-if="selectedSite" v-motion="'fade'" :initial="{ opacity: 0 }" :enter="{ opacity: 1 }" :leave="{ opacity: 0 }"
      class="plantsList-wrapper md:z-0 relative md:w-10/12 md:bottom-10 pb-3 -top-10 md:top-5 flex flex-col  p-1">
      <h2 class="plantsInSite text-lg md:text-2xl mb-2  pl-5">Plants in {{ selectedSite.name }}</h2>
      <div class="plantsList-content  flex flex-col h-[60vh] md:h-[70vh] overflow-y-scroll mx-5" v-if="selectedSite">
        <div class="myplant-card  flex my-1 mx-2 flex-wrap  py-5  gap-2 justify-center">
          <div class="myplant  divide-y-2 divide-orange-900 relative h-auto p-2 w-70  md:w-60"
            v-for="(plant) in filteredPlants" :key="plant.id">
  ...
  y: 0,
              ...
              <div class="relative flex flex-col  md:justify-center " v-if="plant.siteId === selectedSite.id">
                <div class="plant-details ">
                  <div class="flex justify-between">
                    <h2 class="text-1xl md:text-2xl pl-1 capitalize truncate z-10 top-5"><strong>{{ plant.plant_nickname
                    }}</strong></h2>
                ...
                <div class="plant-details-bottom capitalize m-1 ">
    ...
    <transition v-if="isDeleteConfirmationOpen" v-motion :initial="{
      backgroundColor: '$lightgray',
      opacity: 0,
      y: 0,
    }" :variants="{ custom: { scale: 1 } }" :enter="{
  ...
  y: 0,
    ...
    <transition 
      ...
      v-motion :initial="{ backgroundColor: '$lightgray', opacity: 0, y: 0, }" 
...
// Load plant data and update when there are changes
...
// Select a site and filter associated plants
...
// Computed property to filter plants based on the selected site
...
// Confirm and handle site deletion
...
const deleteSiteConfirmation = async () => {
  if (selectedSiteToDelete.value) {
    try {
      ...
      selectedSiteToDelete.value = null;
      isSiteDeleteConfirmationOpen.value = false;
      toast.success("Your site has been deleted!", {
...
// Load site and plant data on component mount
onMounted(async () => {
  loadSites();
  loadPlants();
...
.plantsList-wrapper {
  ...
  .plantsList-content {
    ...
    background: linear-gradient(to top, #eaeaeaa2, #DBDBDB, #F2F2F2, #c3c1ba6f);
    ...
    .myplant-card {
      ...
      .myplant {
        ...
        .plant-details-bottom {
          box-shadow: rgba(55, 55, 55, 0.15) 1.95px 1.25px 2.6px;
        }
